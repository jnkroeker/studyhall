...
func genToken() error {
	...
	// almost any PEM file and prevents shenaniganse like linking the file
	...
	// jti (JWT ID): Unique identified; can be used to prevent the JWT from being replayer (allows a token to be used only once)
	claims := struct {
		jwt.StandardClaims
		Roles []string
	}{
