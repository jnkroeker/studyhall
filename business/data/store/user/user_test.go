...
import (
	...
	"jnk-ardan-service/business/data/tests"
	...
	t.Log("Given the need to work with User records.")
	{
		...
		t.Logf("\tTest %d:\tWhen handling a single User.", testID)
		{
			...
			if saved.Name != *upd.Name {
				...
				t.Logf("\t%s\tTest %d:\tShould be able to se updates to Name.", tests.Success, testID)
			}
			...
			if saved.Email != *upd.Email {
				...
				t.Logf("\t%s\tTest %d:\tShould be able to se updates to Email.", tests.Success, testID)
			}
