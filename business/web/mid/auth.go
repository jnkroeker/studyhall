...
func Authenticate(a *auth.Auth) web.Middleware {
	...
	m := func(handler web.Handler) web.Handler {
		...
		// Create the handler that will be attached inthe middleware chain.
...
// Authorize validates that and authenticated user has at least one role from a
...
func Authorize(roles ...string) web.Middleware {
	...
	m := func(handler web.Handler) web.Handler {
		...
		// Create the handler that will be attached int he middleware chain.
		h := func(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
